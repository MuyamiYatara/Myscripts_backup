...
def modify_and_copy_file(input_file, output_file, ef):
    ...
    # 查找并修改fermi能
    pattern = r"(\$2\s*-\s*)'?\d+\.\d+'?"   # 捕获 $2 - 前缀，数字部分可能有单引号
    fermi_str = f"'{ef}'"  # ef 是你要替换的数字

    # 对 new_lines 每行都执行替换
    for i, line in enumerate(new_lines):
        new_lines[i] = re.sub(pattern, r"\1" + fermi_str, line)
    # 将修改后的内容写回文件B
...
if (autowr == 1) :
    ...
    if (SCF_para['ISPIN'] == '2') and (SCF_para['LSORBIT'] == '.FALSE.'):
        ...
        sp.run(["cp","./"+sbatch_script_name,"./SOC_SCF/"])
