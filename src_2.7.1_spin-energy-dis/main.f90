...
!--------+--------+--------+--------+--------+--------+--------+------!
  ...
  program main
   ...
   if (BulkBand_cube_calc) then
     ...
     !@@@@@
     !> calculate spin distribution of different chemical potentials
     if (Spin_distribution_energy_calc)then
      if(cpuid.eq.0)write(stdout, *)' '
        if(cpuid.eq.0)write(stdout, *)'>> Start of calculating the spin distribution of different chemical potentials'
        call now(time_start)
        call spin_distribution
        call now(time_end)
        call print_time_cost(time_start, time_end, 'Spin Distribution')
        if(cpuid.eq.0)write(stdout, *)'End of calculating the spin distribution of different chemical potentials'
     endif
     call now(time_end)
     !@@@@@
