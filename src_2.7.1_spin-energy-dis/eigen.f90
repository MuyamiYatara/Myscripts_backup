...
!    right eigenvectors
  ...
  subroutine zheevx_pack(JOBZ, UPLO, N, il, iu, A, eigval, eigvec, info)
     use para, only : dp
     implicit none
     ...
     complex(dp), intent(in) :: A(N, N) !> the input hermite complex matrix
     ...
     call zheevx(JOBZ,'I',UPLO,N,A,N,vl,vu,il,iu,abstol,&
         mdim,eigenvalues,eigvec,N,work1,-1,rwork,iwork,ifail,info)
     lwork=int(work1(1))
     work = work1
