        ...
        do imu = 1, OmegaNum 
            do i = 1, Num_wann
                sz_energy_filtered(imu)= sz_energy_filtered(imu) + &
                    dot_product(UU(:, i), matmul(spin_sigma_z, UU(:, i)))*delta(0.001, (W(i)-mu_array(imu))) 
            end do
        end do
...
!> sum over all the kpoints to get the mu dependent spin distribution
    ...
    !> write script for gnuplot
     ...
     if (cpuid==0) then
        ...
        write(outfileindex, '(5a)')"'", 'spin_distribution.dat', "' u 1:4 w l title '{/Symbol s}_{z}' lc rgb 'orange' lw 4 "
        close(outfileindex)
     endif
