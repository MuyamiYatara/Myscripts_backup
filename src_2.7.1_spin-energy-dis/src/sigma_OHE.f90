...
!-----------------------------------------------------------!
     ...
     !> gather Enk and velocity 
      ...
      if (cpuid.eq.0) then
         do ie = 1, OmegaNum
            write(muname, '(f12.5)')mu_array(ie)/eV2Hartree
   ...
   subroutine velocity_calc_iband2(iband, k, velocity_k, E_iband)
      ...
      call zheevx_pack('V', 'U', Num_wann, iband, iband, Hamk_bulk, W, UU, info_flag)
   ...
   subroutine velocity_calc_iband(iband, k, velocity_k)
      ...
      ! calculation bulk hamiltonian
      call ham_bulk_latticegauge(k, Hamk_bulk,0)
...
!     endif
      ...
      if (info_flag/=0) then
         !@@@@@
          !if (cpuid_t == 0) write(*,*)  "N", N,A ,"k",k
         Hamk_bulk= 0d0
         !write(stdout,*) "#############################"
         call ham_bulk_latticegauge(k, Hamk_bulk, 1)
         print *, ' Error info in zheevx: 1231313123', info_flag 
         !if ( cpuid == 0 ) then 
         do i=1, Num_wann
            do j=1, Num_wann
               write(*,*) Hamk_bulk(i,j)
