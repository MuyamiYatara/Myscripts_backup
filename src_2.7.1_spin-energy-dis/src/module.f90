  ...
  module para
     ...
     !@@@@@
     logical :: Spin_distribution_energy_calc ! Flag for spin distribution
     !@@@@@
     ...
     namelist / Control / BulkBand_calc, BulkFS_calc,  BulkFS_Plane_calc, &
                          ...
                          valley_projection_calc, Matrix_Element_calc, BdGChern_calc, SlabBdG_calc, Spin_distribution_energy_calc
...
!@@@@@
subroutine spin_distribution
   use wmpi
   use para

   implicit none
   integer :: knv3, nkx, nky, nkz, ierr, nwann
   integer :: ia, ik, i, j, h, i1, i2, ig, imu
   
   real(dp) :: mu_array(OmegaNum)
   real(dp) :: k(3)
   real(dp) :: time_start, time_end
   real(dp), allocatable :: W(:)
   real(dp), allocatable :: kxyz(:,:), kxyz_shape(:,:)
   real(dp), allocatable :: sx_selected(:, :), sy_selected(:, :), sz_energy_filtered(:)
   real(dp), allocatable :: sx_selected_mpi(:, :), sy_selected_mpi(:, :), sz_energy_filtered_mpi(:)
   !> spin 1/2 Pauli matrix in Wannier basis
   complex(Dp),allocatable :: spin_sigma_x(:,:), spin_sigma_y(:,:), spin_sigma_z(:,:)
   complex(dp), allocatable :: UU(:, :), Hamk_bulk(:,:)

   !> delta function
   real(dp), external :: delta, norm


   !> initialize the 3d bulk kpoints
   knv3=Nk1*Nk2*Nk3
   allocate(kxyz(3, knv3))! kxyz stores the kvectors in fractional coordinates
   allocate(kxyz_shape(3, knv3))! kxyz_shape stores the kvectors in Cartisian coordinates 
   kxyz=0d0
   kxyz_shape=0d0
   ik =0
   do i= 1, Nk1
      do j= 1, Nk2
           do h= 1, Nk3
               ik =ik +1
               kxyz(:, ik)= K3D_start+ K3D_vec1*(i-1)/dble(Nk1-1)+ K3D_vec2*(j-1)/dble(Nk2-1) &
               +K3D_vec3*(h-1)/dble(Nk3-1) -(K3D_vec1+K3D_vec2+K3D_vec3)/2d0
               kxyz_shape(:, ik)= kxyz(1, ik)* Origin_cell%Kua+ kxyz(2, ik)* Origin_cell%Kub+ kxyz(3, ik)* Origin_cell%Kuc 
   ...
   !> allocate the arrays used for calculation
   ...
   if (SOC>0) then
       ...
       allocate( sz_energy_filtered(OmegaNum))
       ...
       allocate( sz_energy_filtered_mpi(OmegaNum))
       ...
       sz_energy_filtered= 0d0; sz_energy_filtered_mpi= 0d0
   ...
   !> generate the spin operator matrix in wannier basis
   if (SOC>0) then
   ...
   !> started to calculate the spin distribution for every chemical potential posint using mpi
   time_start= 0d0
   time_end= 0d0
      
    do ik= 1+cpuid, knv3, num_cpu
        if (cpuid==0.and. mod(ik/num_cpu, 500)==0) &
            write(stdout, *) 'spin_distribution, ik ', ik, 'knv3',knv3, 'time left', &
            (knv3-ik)*(time_end- time_start)/num_cpu, ' s'
        call now(time_start)

        k = kxyz(:, ik)

        !> calculate bulk hamiltonian and eigen vectors
        Hamk_bulk= 0d0
        ...
        !> calculate spin-distribution in every chemical potential point
        !> spin z
        do imu = 1, OmegaNum 
            do i = 1, Num_wann
                sz_energy_filtered(imu)= sz_energy_filtered(imu) + &
                    dot_product(UU(:, i), matmul(spin_sigma_z, UU(:, i)))*delta(Fermi_broadening, (W(i)-mu_array(imu))) 
            end do
        end do
...
!> sum over all the kpoints to get the mu dependent spin distribution
#if defined (MPI)
    call mpi_allreduce(sz_energy_filtered,sz_energy_filtered_mpi,size(sz_energy_filtered),&
                             mpi_dp,mpi_sum,mpi_cmw,ierr)
#else
    sz_energy_filtered_mpi= sz_energy_filtered
#endif
   
    !> kspace volume    
    sz_energy_filtered_mpi = sz_energy_filtered_mpi/dble(knv3)/Origin_cell%CellVolume

    !> write out the results
    outfileindex= outfileindex+ 1
    if (cpuid==0)then
        open(unit=outfileindex, file='spin_distribution.dat')
        write(outfileindex, '("#",a13, 20a16)')'Eenergy (eV)', 'Sz'

        do imu=1, OmegaNum
            write(outfileindex, '(200E16.8)')mu_array(imu)/eV2Hartree, sz_energy_filtered_mpi(imu)
        enddo
    ...
    !> write script for gnuplot
     outfileindex= outfileindex+ 1
     if (cpuid==0) then
        open(unit=outfileindex, file='plot_spinz_distribution.gnu')
        write(outfileindex, '(a)') 'set terminal pdf enhanced color font ",20"'
        write(outfileindex, '(a)')"set output 'spinz_distribution.pdf'"
        ...
        write(outfileindex, '(a)')'set ylabel "Sz Average"'
        write(outfileindex, '(5a)')"plot '", 'spin_distribution.dat', "' u 1:2 w l title '{/Symbol s}_{z}' lc rgb 'orange' lw 4 "
        close(outfileindex)
     endif

     deallocate(Hamk_bulk, W, UU)
     deallocate( sx_selected)
     deallocate( sy_selected)
     deallocate( sz_energy_filtered)
     deallocate( sx_selected_mpi)
     deallocate( sy_selected_mpi)
     deallocate( sz_energy_filtered_mpi)
     deallocate(spin_sigma_x, spin_sigma_y, spin_sigma_z)
    
    
     return
end subroutine spin_distribution
!@@@@@