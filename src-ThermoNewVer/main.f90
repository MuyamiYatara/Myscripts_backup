...
!--------+--------+--------+--------+--------+--------+--------+------!
  ...
  program main
   ...
   if (BulkBand_cube_calc) then
     ...
     !> calculate thermomagnetic transport coefficients
     if (thermag_calc) then
      if(cpuid.eq.0)write(stdout, *)' '
      if(cpuid.eq.0)write(stdout, *)'>> Start of calculating the thermomagnetic transport coefficients'
      call now(time_start)
      call thermomag
      ! call two_band_seebeck
      ! call single_band_seebeck
      ! call zerofield_seebeck
      ! call ode45
      call now(time_end)
      call print_time_cost(time_start, time_end, 'thermag_calc')
      if(cpuid.eq.0)write(stdout, *)'End of calculating the thermomagnetic transport coefficients'
  endif 
