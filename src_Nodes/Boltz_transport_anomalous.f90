       ...
       !call dHdk_latticegauge_Ham(k, W, UU, Vmn_Ham)
        ...
        do ieta= 1, NumberofEta
           ...
           do ie=1, NumOfmu
              mu = mulist(ie)
              !@@@@@
              do m= 1,NumberofSelectedBands

                 everybandindex = Selected_band_index(m)
                 Omega_BerryCurv_t(everybandindex, :)= Omega_BerryCurv(everybandindex, :)*fermi(W(everybandindex)-mu, Beta_fake)
              enddo
