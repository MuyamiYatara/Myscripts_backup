   subroutine FindNodes
      ...
      knv3= Nk1*Nk2*Nk3
      allocate(bandind_list(knv3))
      allocate(kabc_minimal(3, knv3))
      allocate(gap_minimal(knv3))
      allocate(bandind_list_mpi(knv3))
      ...
      do ik=1+cpuid, knv3, num_cpu
         ...
         !@@@@@ k0 bandind 
         call func_bandind(k, bandind)
         write(*,*) bandind
         call FindNode_k0(k, k_out, gap_out, bandind)
         bandind_list(ik)= bandind
      ...
      kabc_minimal_mpi= 0
      gap_minimal_mpi= 0
      call mpi_reduce(bandind_list, bandind_list_mpi, size(bandind_list), &
                      mpi_integer, mpi_sum, 0, mpi_cmw, ierr)
      ...
      if (cpuid==0)then
         ...
         write(outfileindex, '("#", A10, 80A14)') 'kx', 'ky', 'kz', 'gap', 'E', 'bandind', 'k1', 'k2', 'k3'
         ...
         do ik=1, Nleft
           !if (gap_minimal_mpi(ik)< Gap_threshold.and.In_KCUBE(kabc_minimal_mpi(:, ik))) then
            if (gap_minimal_mpi(ik)< Gap_threshold) then
               ...
               write(outfileindex, '(5f14.8, 1I , 10f14.8)') k_cart*Angstrom2atomic, gap_minimal_mpi(ik)/eV2Hartree, &
                  func_energy(kabc_minimal_mpi(:, ik), bandind_list_mpi(ik))/eV2Hartree, bandind_list_mpi(ik), kabc_minimal_mpi(:, ik)
            endif
         enddo
         close(outfileindex)
      endif
      ...
      if (cpuid==0) then
         ...
         !> write script for gnuplot
         outfileindex= outfileindex+ 1
         open(unit=outfileindex, file='Nodes_dis.gnu')
         write(outfileindex, '(a)')"set terminal pngcairo size 800,400 enhanced font 'Arial,12'"
         write(outfileindex, '(a)')"set output 'distribution_of_nodes.png'"
         write(outfileindex, '(a)')"bin_width = 0.01"
         write(outfileindex, '(a)')"offset = bin_width * 0.5"
         write(outfileindex, '(a)')"bin1(x) = bin_width * floor(x/bin_width) - offset + bin_width/2.0"
         write(outfileindex, '(a)')"bin2(x) = bin_width * floor(x/bin_width) + offset + bin_width/2.0"
         write(outfileindex, '(a)')"set style fill solid 0.5"
         write(outfileindex, '(a)')'set boxwidth bin_width'
         write(outfileindex, '(a)')'set xrange [-0.8:0.8]'
         write(outfileindex, '(a)')'set xlabel "E(eV)"'
         write(outfileindex, '(a)')'set ylabel "Count"'
         write(outfileindex, '(a)')'set title "Distribution of Nodes"'
         write(outfileindex, '(a)')'set key top right'
         write(outfileindex, '(a)')'plot "Nodes.dat" using (bin2($5)):(1) smooth freq with boxes lc rgb "orange" title "count"'
         close(outfileindex)
      ...
      deallocate(bandind_list)
      deallocate(kabc_minimal)
      deallocate(gap_minimal)
      deallocate(bandind_list_mpi)
   ...
   subroutine FindNode_k0(k0, k_out, gap_out, bandind)
      ...
      do i=1, npara+1
         gap(i)= func_gap(npara, k(i, :), bandind)  !@@@@@
      enddo

      ptol=1d-5 
      call Proteus(npara, k, gap, ptol, func_gap, bandind, iter)

      x= k(1, :)
      gap_out= func_gap(npara, x, bandind) !@@@@@
   ...
   function func_energy(X, bandind)
      ...
      integer,intent(in) :: bandind !@@@@ the band index we considered
   ...
   subroutine func_bandind(k_ini, bandind)
      ! this function gives the band index near the fermi energy 

      use para
      implicit none

      real(Dp),intent(in):: k_ini(3) ! k point coordinates
      integer :: bandind
      real(dp), allocatable :: W(:)
      complex(dp), allocatable :: Hamk_bulk(:, :)
      integer :: i
      ...
      ! generate bulk Hamiltonian
      if (index(KPorTB, 'KP')/=0)then
         call ham_bulk_kp  (k_ini, Hamk_bulk)
      else
         call ham_bulk_atomicgauge     (k_ini, Hamk_bulk)
      endif

      ! diagonalization by call zheev in lapack
      W= 0d0
      call eigensystem_c( 'N', 'U', Num_wann ,Hamk_bulk, W)
        ! 从小到大扫描，找到第一个 >= -tol 的位置
         do i = 1, num_wann
            if (W(i) >= 0.000001d0) then
               if (i > 1) then
                  bandind = i - 1   ! 前一个就是低于0最近的
               else
                  bandind = 0       ! 所有能量都 >= -tol，没有负的
               end if
            exit
            end if
         end do
   ...
   function func_gap(N,X, bandind)
      ...
      func_gap= W(bandind+1)- W(bandind)!@@@@
